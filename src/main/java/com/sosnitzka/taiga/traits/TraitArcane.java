...
public class TraitArcane extends AbstractTrait {
    ...
    public void afterHit(ItemStack tool, EntityLivingBase player, EntityLivingBase target, float damage, boolean wasCritical, boolean wasHit) {
        ...
        if (random.nextFloat() <= 0.02 && isNight(time)) {
            ToolHelper.healTool(tool, random.nextInt(3) + 1, null);
        }
    }
    ...
    public void onEntityKill(LivingDeathEvent e) {
        ...
        if (!w.isRemote && e.getSource().getEntity() != null) {
            if (e.getSource().getEntity() instanceof EntityPlayer && e.getEntity() instanceof EntityCreature) {
                ...
                if (isNight((int) w.getWorldTime()) && random.nextFloat() > 0.85 && TinkerUtil.hasTrait(TagUtil.getTagSafe(tool), identifier)) {
                    ToolHelper.healTool(tool, random.nextInt(20), null);
                }
            }
        }
    }
